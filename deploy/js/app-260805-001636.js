...
function lattice(seed, freq) {
const n = freq * freq;
const a = new Float32Array(n);
for (let i = 0; i < n; i++) {
a[i] = hash2(i % freq, (i / freq) | 0, seed) / 4294967296;
}
return a;
}
function sample(lat, freq, x, y) {
const gx = (x & (TS - 1)) / TS * freq;
const gy = (y & (TS - 1)) / TS * freq;
const ix = gx | 0;
const iy = gy | 0;
const fx = gx - ix;
const fy = gy - iy;
const lat2 = lat;
const f2 = freq;
const v00 = lat2[(ix % f2) + (iy % f2) * f2];
const v10 = lat2[((ix + 1) % f2) + (iy % f2) * f2];
const v01 = lat2[(ix % f2) + ((iy + 1) % f2) * f2];
const v11 = lat2[((ix + 1) % f2) + ((iy + 1) % f2) * f2];
return v00 * (1 - fx) * (1 - fy) + v10 * fx * (1 - fy) + v01 * (1 - fx) * fy + v11 * fx * fy;
}
function mkTex(name, icon, seed, lava, color) {
return {
name: name, icon: icon, seed: seed, lava: !!lava,
n1: lattice(seed + 1, 12),
n2: lattice(seed + 2, 36),
color: color
};
}
const T = {};
T.travnik = mkTex('Trávník', '🌿', 11, false, function (n1, n2, x, y, rel) {
const v = n1 - 0.5;
let r, g, b;
if (rel <= 9) {
r = 96 + v * 60;
g = 164 + v * 46;
b = 58 + v * 30;
if (hash2(x, y, 3) % 6 === 0) { r = 64; g = 128; b = 44; }
const blade = hash2(x, y, 17) % 3;
if (blade === 1) { r += 24; g += 22; }
else if (blade === 2) { g += 30; }
if (hash2(x, y, 5) % 61 === 0) { r = 248; g = 238; b = 168; }
if (hash2(x, y, 19) % 37 === 0) { r *= 0.82; g *= 0.82; b *= 0.82; }
} else {
r = 118 + v * 56;
g = 94 + v * 42;
b = 60 + v * 30;
if (hash2(x, y, 7) % 11 === 0) { r = 92; g = 74; b = 48; }
if (hash2(x, y, 13) % 29 === 0) { r = 150; g = 134; b = 110; }
if (hash2(x, y, 23) % 41 === 0) { r = 70; g = 56; b = 38; }
if (n2 > 0.72 && hash2(x, y, 31) % 3 !== 0) { r *= 0.78; g *= 0.76; b *= 0.74; }
}
out[0] = r | 0; out[1] = g | 0; out[2] = b | 0;
return out;
});
T.pisky = mkTex('Poušť', '🏜️', 22, false, function (n1, n2, x, y, rel) {
const v = n1 - 0.5;
let r, g, b;
if (rel <= 7) {
const d = Math.sin((x & 127) / 128 * Math.PI * 5 + n1 * 8 + (y & 127) / 128 * Math.PI * 3);
r = 226 + v * 40;
g = 198 + v * 36;
b = 138 + v * 28;
if (d > 0.5) { r += 22; g += 20; b += 14; }
else if (d < -0.5) { r -= 20; g -= 18; b -= 12; }
if (hash2(x, y, 7) % 23 === 0) { r *= 0.92; g *= 0.92; b *= 0.92; }
} else {
const band = Math.floor((y + n1 * 40) / 14) % 3;
r = 196 + v * 44;
g = 168 + v * 40;
b = 112 + v * 30;
if (band === 1) { r *= 0.88; g *= 0.88; b *= 0.88; }
else if (band === 2) { r *= 0.76; g *= 0.76; b *= 0.76; }
if (hash2(x, y, 11) % 17 === 0) { r = 170; g = 140; b = 92; }
if (n2 > 0.78) { r *= 0.9; g *= 0.9; b *= 0.9; }
}
out[0] = r | 0; out[1] = g | 0; out[2] = b | 0;
return out;
});
T.snih = mkTex('Sníh', '❄️', 33, false, function (n1, n2, x, y, rel) {
const v = n1 - 0.5;
let r, g, b;
if (rel <= 12) {
r = 238 + v * 26;
g = 244 + v * 26;
b = 250 + v * 26;
if (hash2(x, y, 9) % 43 === 0) { r = 255; g = 255; b = 255; }
if (hash2(x, y, 15) % 71 === 0) { r *= 0.86; g *= 0.88; b *= 0.9; }
} else {
r = 148 + v * 42;
g = 176 + v * 40;
b = 214 + v * 36;
if (hash2(x, y, 13) % 5 === 0) { r *= 0.62; g *= 0.68; b *= 0.78; }
if (hash2(x, y, 21) % 19 === 0) { r += 36; g += 40; b += 34; }
if (n2 < 0.15 && hash2(x, y, 27) % 4 !== 0) { r *= 0.7; g *= 0.76; b *= 0.86; }
}
out[0] = r | 0; out[1] = g | 0; out[2] = b | 0;
return out;
});
T.sopka = mkTex('Sopka', '🌋', 44, true, function (n1, n2, x, y, rel) {
const v = n1 - 0.5;
let r, g, b;
if (rel <= 8) {
r = 96 + v * 46;
g = 88 + v * 42;
b = 94 + v * 42;
if (hash2(x, y, 11) % 9 === 0) { r = 222; g = 108; b = 40; }
if (n2 > 0.8) { r *= 0.6; g *= 0.6; b *= 0.62; }
if (hash2(x, y, 25) % 13 === 0) { r = 60; g = 56; b = 62; }
} else {
r = 64 + v * 40;
g = 60 + v * 38;
b = 68 + v * 38;
if (n2 > 0.86 && hash2(x, y, 11) % 5 !== 0) { r = 226; g = 112; b = 42; }
if (n2 < 0.08 && hash2(x, y, 13) % 3 !== 0) { r = 40; g = 38; b = 44; }
if (hash2(x, y, 29) % 47 === 0) { r = 140; g = 90; b = 60; }
}
out[0] = r | 0; out[1] = g | 0; out[2] = b | 0;
return out;
});
T.kamen = mkTex('Skály', '🪨', 55, false, function (n1, n2, x, y, rel) {
const v = n1 - 0.5;
let r, g, b;
if (rel <= 8) {
r = 136 + v * 70;
g = 140 + v * 66;
b = 148 + v * 62;
const band = Math.floor((y + n1 * 30) / 12) % 3;
if (band === 1) { r += 30; g += 30; b += 28; }
else if (band === 2) { r -= 26; g -= 26; b -= 24; }
if (hash2(x, y, 15) % 31 === 0) { r *= 0.66; g *= 0.66; b *= 0.66; }
} else {
r = 112 + v * 56;
g = 116 + v * 54;
b = 124 + v * 52;
if (hash2(x, y, 21) % 9 === 0) { r *= 0.55; g *= 0.55; b *= 0.55; }
if (hash2(x, y, 27) % 23 === 0) { r += 44; g += 44; b += 42; }
if (n2 > 0.82) { r *= 0.72; g *= 0.72; b *= 0.72; }
}
out[0] = r | 0; out[1] = g | 0; out[2] = b | 0;
return out;
});
for (const k in T) T[k].key = k;
function terrainColor(key, x, y, rel) {
const tex = T[key] || T.travnik;
const n1 = sample(tex.n1, 12, x, y);
const n2 = sample(tex.n2, 36, x, y);
tex.color(n1, n2, x, y, rel);
if (rel <= 3) {
out[0] = clamp(out[0] + 14, 0, 255);
out[1] = clamp(out[1] + 14, 0, 255);
out[2] = clamp(out[2] + 14, 0, 255);
} else if (rel > 70) {
const k = clamp((rel - 70) / 130, 0, 1);
out[0] = clamp(out[0] * (1 - 0.32 * k), 0, 255);
out[1] = clamp(out[1] * (1 - 0.32 * k), 0, 255);
out[2] = clamp(out[2] * (1 - 0.32 * k), 0, 255);
}
if (tex.lava && rel >= 90 && hash2(x, y, 7) % 37 === 0) {
out[0] = 255; out[1] = 150; out[2] = 52;
}
return out;
}
return { terrainColor, data: T, get keys() { return Object.keys(T); } };
})();
const Sfx = (() => {
let ctx = null;
function ensure() {
...
let texKey = 'travnik';
const spawnCols = [0.10, 0.17, 0.24, 0.31, 0.69, 0.76, 0.83, 0.90].map(function (f) { return f * CFG.W; });
...
const rel = y - surface[x];
const c = TEXTURES.terrainColor(texKey, x, y, rel);
...
function fillRectData(x0, y0, x1, y1) {
for (let y = clamp(Math.floor(y0), 0, CFG.WATER_Y - 1); y <= clamp(Math.ceil(y1), 0, CFG.WATER_Y - 1); y++) {
for (let x = clamp(Math.floor(x0), 0, CFG.W - 1); x <= clamp(Math.ceil(x1), 0, CFG.W - 1); x++) {
data[idx(x, y)] = 1;
}
}
}
function carveRectData(x0, y0, x1, y1) {
for (let y = clamp(Math.floor(y0), 0, CFG.WATER_Y - 1); y <= clamp(Math.ceil(y1), 0, CFG.WATER_Y - 1); y++) {
for (let x = clamp(Math.floor(x0), 0, CFG.W - 1); x <= clamp(Math.ceil(x1), 0, CFG.W - 1); x++) {
data[idx(x, y)] = 0;
}
...
for (const s of spawnCols) if (Math.abs(x - s) < 120) return true;
return false;
}
function genBase() {
const n = irand(3, 5);
...
const top = clamp(Math.round(h), 80, 470);
for (let y = top; y < CFG.WATER_Y; y++) data[idx(x, y)] = 1;
}
}
function genVolcano() {
const cx = Math.round(CFG.W * 0.5);
const baseY = topAt(cx);
if (baseY < 0) return;
const peakY = Math.max(baseY - 170, 100);
const wHalf = 130;
for (let x = cx - wHalf; x <= cx + wHalf; x++) {
const t = (x - (cx - wHalf)) / (2 * wHalf);
const hgt = Math.sin(t * Math.PI);
const top = Math.round(baseY - hgt * (baseY - peakY) * 0.85);
for (let y = top; y < baseY + 30; y++) data[idx(x, y)] = 1;
}
carveData(cx, peakY + 6, 22);
carveData(cx, peakY + 26, 12);
}
function genCastle() {
const cx = Math.round(CFG.W * 0.5);
let minTop = Infinity;
for (let x = cx - 280; x <= cx + 280; x++) {
const t = topAt(x);
if (t >= 0 && t < minTop) minTop = t;
}
if (minTop === Infinity) return;
const baseTop = clamp(minTop + 60, 160, CFG.WATER_Y - 160);
const wallH = 76;
const wallTh = 16;
const wHalf = 165;
const topY = baseTop - wallH;
const lx = cx - wHalf;
const rx = cx + wHalf;
fillRectData(lx - wallTh, topY, rx + wallTh, baseTop + 40);
const tw = 26;
for (let s = -1; s <= 1; s += 2) {
const tx = cx + s * wHalf;
fillRectData(tx - tw, topY - 24, tx + tw, baseTop + 40);
fillRectData(tx - tw - 4, topY - 40, tx - tw + 12, topY - 24);
fillRectData(tx + tw - 12, topY - 40, tx + tw + 4, topY - 24);
}
for (let bx = lx - wallTh; bx <= rx + wallTh; bx += 26) {
if (Math.abs(bx - cx) < 30) continue;
fillRectData(bx, topY - 14, bx + 12, topY);
}
fillMound(cx, baseTop, wHalf * 0.85);
carveRectData(cx - 15, topY + 14, cx + 15, baseTop + 40);
carveData(cx, topY + 14, 15);
}
function fillMound(cx, baseTop, r) {
const x0 = clamp(Math.floor(cx - r), 0, CFG.W - 1);
const x1 = clamp(Math.ceil(cx + r), 0, CFG.W - 1);
const y0 = clamp(Math.floor(baseTop), 0, CFG.WATER_Y - 1);
const y1 = clamp(Math.ceil(baseTop + r), 0, CFG.WATER_Y - 1);
const r2 = r * r;
for (let y = y0; y <= y1; y++) {
for (let x = x0; x <= x1; x++) {
const dx = x - cx, dy = y - baseTop;
if (dx * dx + dy * dy <= r2) data[idx(x, y)] = 1;
}
}
}
...
const n = irand(2, 4);
for (let i = 0; i < n; i++) {
const x = irand(60, CFG.W - 60);
if (nearSpawn(x) || Math.abs(x - CFG.W * 0.5) < 260) continue;
const sy = topAt(x);
if (sy < 0) continue;
carveData(x, sy + irand(0, 14), irand(20, 38));
}
}
function genRidges() {
const n = irand(2, 4);
...
if (nearSpawn(x) || Math.abs(x - CFG.W * 0.5) < 280) continue;
const sy = topAt(x);
if (sy < 0) continue;
const r = irand(30, 60);
fillData(x, sy - irand(18, 38), r);
}
}
function genMap() {
genBase();
if (texKey === 'kamen') genCastle();
else if (texKey === 'sopka') genVolcano();
genRidges();
genCraters();
rebuildSurface(0, CFG.W - 1);
}
function build(key) {
texKey = TEXTURES.data[key] ? key : 'travnik';
...
const sx = Terrain.spawnXs[s[0]];
const x = sx + rand(-14, 14);
...
function nextAliveOf(team, after) {
const alive = state.worms.filter(function (w) { return w.team === team && w.alive; });
if (!alive.length) return null;
let i = after ? alive.indexOf(after) : -1;
i = (i + 1) % alive.length;
return alive[i];
}
function startTurn() {
...
const last = state.lastWorm[state.currentTeam];
const nw = nextAliveOf(state.currentTeam, last);
state.selected = nw;
state.lastWorm[state.currentTeam] = nw;
...
for (let y = 0; y < H; y++) {
for (let x = 0; x < W; x++) {
const col = TEXTURES.terrainColor(tex.key, x * 4, y * 4, y * 2.2);
const di = (y * W + x) * 4;
